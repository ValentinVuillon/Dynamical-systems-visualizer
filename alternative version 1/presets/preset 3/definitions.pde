...
class halvorsen_dynamical_system{
  ...
  float a=1.4;
    ...
    for (int i=0; i<steps; i++){
      for(firefly f: fg.group){
        f.velocity.x=-a*f.position.x-4*f.position.y-4*f.position.z-pow(f.position.y,2);
        f.velocity.y=-a*f.position.y-4*f.position.z-4*f.position.x-pow(f.position.z,2);
        f.velocity.z=-a*f.position.z-4*f.position.x-4*f.position.y-pow(f.position.x,2);
        f.position=f.position.add((f.velocity).mult(dt));
